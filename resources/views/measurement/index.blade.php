  ...
  @endpush
            ...
            <div class="row align-items-end fixheader">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>Measurement</h5>
                            <span>View, delete and update Measurement</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    ...
                    <div class="card-header row">
                        ...
                        <div class="col col-sm-5">
                            <div class="card-options text-right">
                                <span class="mr-5" id="top">{{($measurements->currentpage()-1)*$measurements->perpage()+1}} to {{$measurements->currentpage()*$measurements->perpage()}}
                                            of  {{$measurements->total()}} entries</span>
                                <a href="#"><i class="ik ik-chevron-left"></i></a>
                                <a href="#"><i class="ik ik-chevron-right"></i></a>
                                <a class=" btn btn-outline-primary btn-semi-rounded  tradeadd" href="#categoryAdd" data-toggle="modal" data-target="#categoryAdd"><i class="ik ik-plus"></i>&nbsp;Add Measurement</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <table id="product_table" class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ isset($measurement->name) ? $measurement->name :'' }}</td>
                                    
                                    <td>
                                        <a href="javascript:;"  class="measureid"  data-id="{{ isset($measurement->id) ? $measurement->id:'' }}" data-url="{{route('measurement.edit', $measurement->id) }}"><i class="ik ik-edit-2 f-16 mr-15 text-green"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                       
                                        {{--<a href="javascript:;" class="deletebyid" data-id="{{ isset($measurement->id) ? $measurement->id:'' }}"  data-url="{{route('measuredelete',$measurement['id'])}}"><i class="ik ik-trash-2 f-16 text-red"></i></a>--}}
                                    </td>
                                </tr>
                                @endforeach
        ...
        <div class="modal-dialog w-300" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="categoryAddLabel">{{ __('Add Measurement')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <form class="forms-sample" method="POST" action="{{ route('measurement.store') }}" enctype= multipart/form-data>
                @csrf
    ...
    <div class="modal fade edit-layout-modal pr-0 " id="categoryView" tabindex="-1" role="dialog" aria-labelledby="categoryViewLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog w-300" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="categoryViewLabel">{{ __('Edit Measurement')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <form class="forms-sample" method="GET" action="{{ route('updatemeasure') }}" enctype= multipart/form-data >
...
<script type="text/javascript">
    var route = "{{ url('measurementsearch') }}";
 $('#search').typeahead({
